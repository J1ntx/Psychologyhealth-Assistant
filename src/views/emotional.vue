<template>
  <div>
    ...
    <el-table :data="tableData" style="width:100%">
      ...
      <el-table-column label="会话ID" width="160">
        <template #default="{ row }">
          <el-avatar>{{ row.nickname }}</el-avatar>
        </template>
      </el-table-column>
      <el-table-column prop="diaryDate" label="记录日期" width="200"></el-table-column>
      <el-table-column prop="createAt" label="情绪评分">
        <template #default="{ row }">
      ...
      <el-table-column label="生活指标" width="160">
        <template #default="{ row }">
          <div>
            <p>
              睡眠：{{ row.sleepQuality }} / 5
            </p>
            <p>
              压力:{{ row.stressLevel }} / 5
            </p>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="情绪触发因素" prop="emotionTriggers" width="160"></el-table-column>
      <el-table-column label="日记内容" prop="diaryContent" width="300"></el-table-column>
    ...
    <el-pagination style="margin-top:25px" :page-size="pagination.size" :page-sizes="[2, 4, 6, 8, 10]"
      layout="prev,pager,next,->,sizes" :total="pagination.total" @current-change="handleCurrentChange"
...
const handleSearch = async (formData) => {
  ...
  const result = await getEmotionalPage(params)
...
const handleSizeChange = (val) => {
  ...
  pagination.value.current = 1 // 切换条数 → 回到第1页
  handleSearch()
}
// 详情
