...
一个基于 `Vue 3`、`Vite` 和 `Element Plus` 构建的心理健康平台前端项目，包含用户端与后台管理端两套界面，围绕“AI 心理咨询 + 情绪记录 + 心理健康知识内容管理”展开。

该项目适合作为 Vue 3 综合练习项目、课程设计、个人作品集或面试展示项目。
...
- `Vue Router` - 路由管理
- `WangEditor` - 富文本编辑器
- `@microsoft/fetch-event-source` - SSE 流式消息通信
...
项目使用 `@microsoft/fetch-event-source` 与后端 `text/event-stream` 接口通信，实现 AI 回复逐段渲染的效果，更贴近真实 AI 聊天产品体验。

### 后台内容管理

后台知识文章模块支持文章分页、编辑、状态更新、删除、封面上传与富文本编辑，形成完整的内容管理闭环。
...
- 如需访问后端接口文档，可搜索 `https://xsl1e23zpk.apifox.cn/`
